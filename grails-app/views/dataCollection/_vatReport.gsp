...
<table cellpadding="0" cellspacing="0" border="3px;" class="display" id="example"
       style="margin: 5px;width: 99%;border: 1px solid #000000" align="center">
    <thead>
    <tr>
        <td><center>S.No</center></td>
        <td><center>District Office Name</center></td>
        <g:if test="${districtOfficeList}">
        </g:if>
        <td><center>VAT Collection (Rs.)</center></td>
    </tr>
    </thead>
    <tbody>
    <g:each in="${districtOfficeList}" var="subDiv" status="i">
        <tr name="dataTr">
            ...
            <g:if test="${districtOffice}">
                ...
                <td name="vATCollected">${subDiv[1]}</td>
            </g:if>
            ...
            <g:else>
                <td name="vATCollected">${subDiv[0]}</td>
            </g:else>
        </tr>
    </g:each>
    </tbody>
...
<script type="text/javascript">
     ...
     */
    $('td[name="vATCollected"]').live('click', function() {
        var value;
        ...
        $('#popUpOuterDiv h2 span p').html('VAT Collection (Rs.)');
